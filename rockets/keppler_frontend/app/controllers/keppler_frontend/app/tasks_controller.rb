...
module KepplerFrontend
  class App::TasksController < App::FarmsController
    ...
    before_action :set_task, only: %i[show edit update destroy assign_cattle delete_assignment]
    ...
    def show
      @cows = @task.cows.order(:serie_number).page(params[:page])
      @assign = KepplerCattle::Location.new
      # respond_to_formats(@task)
    end

    def new
      @task = KepplerFarm::Task.new
    end
    ...
    def create
      ...
      if @task.save!
        redirect_to farm_task_path(@farm, @task)
      else
    ...
    def delete_assignment
      if params[:multiple_ids].blank?
        ...
        params[:multiple_ids].remove("[", "]").split(',').each do |id|
          ...
          if location&.exists?
            counter += 1 if location.update(task_id: nil)
          end
        end
      end
      if counter.zero?
      ...
      redirect_to action: :show, farm_id: @farm&.id, id: @task.id
    end
    ...
    def index_variables
      @q = KepplerFarm::Task.ransack(params[:q])
      set_task = @q.result(distinct: true)
      @tasks = set_task.page(@current_page).order(position: :desc)
      @total = @tasks.size
      @attributes = KepplerFarm::Task.index_attributes
    end
